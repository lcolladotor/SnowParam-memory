...
In this example, the memory mean fold change when using `SnowParam()` between R 3.2.x and 3.1.x is `r signif(mean(df2$memory[c(1, 10, 19)]) / mean(df2$memory[c(3, 12, 21)]), 3)`x, compared to `r signif(mean(df$memory[c(1, 10, 19)]) / mean(df$memory[c(3, 12, 21)]), 3)`x from the first example. In comparison, with `SerialParam()` the same mean fold changes are `r signif(mean(df2$memory[c(7, 16, 25)]) / mean(df2$memory[c(9, 18, 27)]), 3)`x and `r signif(mean(df$memory[c(7, 16, 25)]) / mean(df$memory[c(9, 18, 27)]), 3)`x respectively.
